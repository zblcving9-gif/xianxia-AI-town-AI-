...
const ResourceSystem = {
    ...
    gather(resource) {
        ...
        if (resource.amount <= 0) {
            ...
            // 移除物理碰撞体（资源耗尽后可以通过）
            if (resource.body) { Core.removeBody(resource.body); resource.body = null; }
    ...
    respawnResource(resource) {
        ...
        // 恢复物理碰撞体（仅树和石头有物理体）
        if (!resource.body && (resource.type === 'tree' || resource.type === 'rock')) {
            const resCollision = { collisionFilter: { category: 0x0004, mask: 0x0001 } };
            resource.body = Core.createStaticCircle(resource.x, resource.y, resource.size * 0.5, `res_${resource.id}`, resCollision);
            Core.addBody(resource.body);
        }
    },
