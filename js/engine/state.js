...
const GameState = {
    ...
    createWorld() {
        ...
        [{ x: 500, y: 1800, w: 400, h: 30 }, { x: 1200, y: 1600, w: 300, h: 30 }, { x: 2000, y: 1700, w: 350, h: 30 }, { x: 2600, y: 1500, w: 250, h: 30 }]
            .forEach(p => Core.addBody(Core.createStatic(p.x, p.y, p.w, p.h, 'platform', { collisionFilter: { category: 0x0002, mask: 0x0001 } })));
    },
    ...
    createResources() {
        const resCollision = { collisionFilter: { category: 0x0004, mask: 0x0001 } };
        for (let i = 0; i < 30; i++) {
            const r = { id: `tree_${i}`, type: 'tree', x: 200 + Math.random() * 2600, y: 300 + Math.random() * 1600, size: 30 + Math.random() * 20, amount: 5 + Math.floor(Math.random() * 10), maxAmount: 15, respawnTime: 0 };
            r.body = Core.createStaticCircle(r.x, r.y, r.size * 0.5, `res_tree_${i}`, resCollision);
            Core.addBody(r.body);
            this.resources.push(r);
        }
        for (let i = 0; i < 20; i++) {
            const r = { id: `rock_${i}`, type: 'rock', x: Math.random() * 2900, y: Math.random() * 1900, size: 25 + Math.random() * 15, amount: 3 + Math.floor(Math.random() * 7), maxAmount: 10, respawnTime: 0 };
            r.body = Core.createStaticCircle(r.x, r.y, r.size * 0.5, `res_rock_${i}`, resCollision);
            Core.addBody(r.body);
            this.resources.push(r);
        }
    ...
    updatePlayer(dt) {
        ...
        // 通过setVelocity移动，让Matter.js处理物理碰撞
        if (p.body) {
            Core.setVelocity(p.body, { x: vx * MOVE_SPEED, y: vy * MOVE_SPEED });
            p.x = p.body.position.x; p.y = p.body.position.y;
        }
        p.velocityX = vx * MOVE_SPEED; p.velocityY = vy * MOVE_SPEED;
